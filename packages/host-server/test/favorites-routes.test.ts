/**
 * The `/library/favorites` + `/library/collections` routes over a `FileLibraryStore` on a real temp
...
describe("favoriting pages", () => {
  test("PUT favorites a page with both re-anchor keys, DELETE removes it", async () => {
    const put = await send("PUT", "/library/favorites/page/demo/s1/c1/3", {
    ...
    expect((await send("DELETE", "/library/favorites/page/demo/s1/c1/3")).status).toBe(200);
    expect(await json<FavoriteBody[]>(await get("/library/favorites"))).toEqual([]);
  });

  test("PUT is idempotent — re-favoriting overwrites rather than duplicating", async () => {
    const first = await json<FavoriteBody>(
      await send("PUT", "/library/favorites/page/demo/s1/c1/0", { seriesTitle: "Old" }),
    );
    const second = await json<FavoriteBody>(
      await send("PUT", "/library/favorites/page/demo/s1/c1/0", { seriesTitle: "New" }),
    );
    ...
    expect(await json<FavoriteBody[]>(await get("/library/favorites"))).toHaveLength(1);

    await send("DELETE", "/library/favorites/page/demo/s1/c1/0");
  });
  ...
  test("the two-PUT flow: favorite on tap, then send the hash, without losing anything", async () => {
    ...
    const first = await json<FavoriteBody>(
      await send("PUT", "/library/favorites/page/demo/twoput/c1/4", {
        seriesTitle: "Two Put",
        chapterName: "Ch 1",
        pageCount: 18,
    ...
    const listed = await json<FavoriteBody[]>(await get("/library/favorites?series=demo:twoput"));
    expect(listed[0]).toMatchObject({ pageCount: 18, sourceUrl: "https://cdn/p4.png", contentHash: "sha-p4" });

    await send("DELETE", "/library/favorites/page/demo/twoput/c1/4");
  });

  test("survives a restart — favorites round-trip through the FileLibraryStore on disk", async () => {
    await send("PUT", "/library/favorites/page/demo/s1/c1/1", { seriesTitle: "Persisted" });
    ...
    expect((await reopened.getFavoriteItems()).map((p) => p.seriesTitle)).toEqual(["Persisted"]);
    await send("DELETE", "/library/favorites/page/demo/s1/c1/1");
  });

  test("rejects a missing seriesTitle and a non-integer pageIndex", async () => {
    expect((await send("PUT", "/library/favorites/page/demo/s1/c1/0", {})).status).toBe(400);
    expect((await send("PUT", "/library/favorites/page/demo/s1/c1/abc", { seriesTitle: "S" })).status).toBe(400);
    expect((await send("PUT", "/library/favorites/page/demo/s1/c1/-1", { seriesTitle: "S" })).status).toBe(400);
    expect((await send("DELETE", "/library/favorites/page/demo/s1/c1/abc")).status).toBe(400);
  });

  test("URL-encoded id segments round-trip, including the chapterless sentinel", async () => {
    const page = await json<FavoriteBody>(
      await send("PUT", `/library/favorites/page/demo/${encodeURIComponent("s/2?x")}/__direct__/5`, {
        seriesTitle: "Direct",
      }),
    );
    ...
    const indices = await json<number[]>(
      await get(`/library/favorites/page/demo/${encodeURIComponent("s/2?x")}/__direct__/indices`),
    );
    expect(indices).toEqual([5]);

    await send("DELETE", `/library/favorites/page/demo/${encodeURIComponent("s/2?x")}/__direct__/5`);
  });

  test("DELETE of a page that was never favorited is a no-op, not a 404", async () => {
    expect((await send("DELETE", "/library/favorites/page/demo/ghost/c1/0")).status).toBe(200);
  });
});
...
describe("chapter indices route", () => {
  test("returns just that chapter's favorited indices, ascending", async () => {
    for (const i of [4, 1, 2]) {
      await send("PUT", `/library/favorites/page/demo/s1/c1/${i}`, { seriesTitle: "S" });
    }
    await send("PUT", "/library/favorites/page/demo/s1/c2/8", { seriesTitle: "S" });

    expect(await json<number[]>(await get("/library/favorites/page/demo/s1/c1/indices"))).toEqual([1, 2, 4]);
    expect(await json<number[]>(await get("/library/favorites/page/demo/s1/c2/indices"))).toEqual([8]);
    expect(await json<number[]>(await get("/library/favorites/page/demo/s1/nope/indices"))).toEqual([]);

    for (const i of [4, 1, 2]) await send("DELETE", `/library/favorites/page/demo/s1/c1/${i}`);
    await send("DELETE", "/library/favorites/page/demo/s1/c2/8");
  });

  test("the indices path is page-typed — a chapter anchor on the same chapter never leaks in", async () => {
    await send("PUT", "/library/favorites/chapter/demo/s1/c1", { seriesTitle: "S" });
    await send("PUT", "/library/favorites/page/demo/s1/c1/2", { seriesTitle: "S" });
    expect(await json<number[]>(await get("/library/favorites/page/demo/s1/c1/indices"))).toEqual([2]);
    await send("DELETE", "/library/favorites/chapter/demo/s1/c1");
    await send("DELETE", "/library/favorites/page/demo/s1/c1/2");
  });
});

describe("reconcile route — chapter drift", () => {
  test("repairs a shifted favorite and returns the indices to trust", async () => {
    await send("PUT", "/library/favorites/page/demo/drift/c1/2", {
      seriesTitle: "Drifty",
      pageCount: 4,
    ...
    const res = await send("POST", "/library/favorites/page/demo/drift/c1/reconcile", {
    ...
    expect(await json<number[]>(await get("/library/favorites/page/demo/drift/c1/indices"))).toEqual([3]);
    await send("DELETE", "/library/favorites/page/demo/drift/c1/3");
  });

  test("an unlocatable favorite is marked stale, kept, and dropped from the indices", async () => {
    await send("PUT", "/library/favorites/page/demo/gone/c1/1", {
    ...
    const res = await json<ReconcileBody>(
      await send("POST", "/library/favorites/page/demo/gone/c1/reconcile", {
    ...
    const all = await json<FavoriteBody[]>(await get("/library/favorites?series=demo:gone"));
    expect(all).toHaveLength(1);
    expect(all[0]).toMatchObject({ stale: true, seriesTitle: "Replaced" });
    expect(await json<number[]>(await get("/library/favorites/page/demo/gone/c1/indices"))).toEqual([]);

    await send("DELETE", "/library/favorites/page/demo/gone/c1/1");
  });

  test("a content hash relocates a favorite whose URLs have rotated", async () => {
    await send("PUT", "/library/favorites/page/demo/rot/c1/2", {
      seriesTitle: "Rotated",
      pageCount: 3,
    ...
    const res = await json<ReconcileBody>(
      await send("POST", "/library/favorites/page/demo/rot/c1/reconcile", {
        pages: [
    ...
    expect(await json<number[]>(await get("/library/favorites/page/demo/rot/c1/indices"))).toEqual([1]);
    await send("DELETE", "/library/favorites/page/demo/rot/c1/1");
  });

  test("an empty page list is a no-op, so a failed fetch can't stale a whole chapter", async () => {
    await send("PUT", "/library/favorites/page/demo/safe/c1/0", { seriesTitle: "S", pageCount: 2 });
    expect(
      await json<ReconcileBody>(await send("POST", "/library/favorites/page/demo/safe/c1/reconcile", { pages: [] })),
    ).toEqual({ indices: [0], repaired: 0, stale: 0 });
    await send("DELETE", "/library/favorites/page/demo/safe/c1/0");
  });

  test("requires a pages array, and tolerates junk entries within it", async () => {
    expect((await send("POST", "/library/favorites/page/demo/s1/c1/reconcile", {})).status).toBe(400);
    expect((await send("POST", "/library/favorites/page/demo/s1/c1/reconcile", { pages: "nope" })).status).toBe(400);
    // One odd element must not reject an entire chapter's reconcile — non-strings become "",
    // which still counts toward the length.
    const ok = await send("POST", "/library/favorites/page/demo/s1/c1/reconcile", {
...
describe("collections", () => {
  ...
  test("membership: assign, filter, and cascade on collection delete", async () => {
    const panels = await json<CollectionBody>(
      await send("POST", "/library/collections", { name: "Panels" }),
    );
    const p1 = await json<FavoriteBody>(
      await send("PUT", "/library/favorites/page/demo/s1/c1/0", { seriesTitle: "Alpha" }),
    );
    const p2 = await json<FavoriteBody>(
      await send("PUT", "/library/favorites/page/demo/s2/c1/0", { seriesTitle: "Beta" }),
    );

    // Addressed by coordinates, like every other favorite route — no id in the URL.
    const assigned = await send("PUT", "/library/favorites/page/demo/s1/c1/0/collections", {
      collectionIds: [panels.id],
    });
    expect(assigned.status).toBe(200);
    expect((await json<FavoriteBody>(assigned)).collectionIds).toEqual([panels.id]);

    const inPanels = await json<FavoriteBody[]>(await get(`/library/favorites?collection=${panels.id}`));
    expect(inPanels.map((p) => p.id)).toEqual([p1.id]);
    const uncollected = await json<FavoriteBody[]>(await get("/library/favorites?collection=uncollected"));
    ...
    await send("DELETE", `/library/collections/${panels.id}`);
    expect(await json<FavoriteBody[]>(await get("/library/favorites"))).toHaveLength(2);
    expect((await json<FavoriteBody[]>(await get("/library/favorites?collection=uncollected"))).length).toBe(2);

    await send("DELETE", "/library/favorites/page/demo/s1/c1/0");
    await send("DELETE", "/library/favorites/page/demo/s2/c1/0");
  });

  test("assigning collections to an unknown favorite 404s; a bad body or index 400s", async () => {
    expect((await send("PUT", "/library/favorites/page/demo/ghost/c1/0/collections", {})).status).toBe(400);
    expect((await send("PUT", "/library/favorites/page/demo/ghost/c1/x/collections", { collectionIds: [] })).status).toBe(400);
    expect(
      (await send("PUT", "/library/favorites/page/demo/ghost/c1/0/collections", { collectionIds: [] })).status,
    ).toBe(404);
  });
});
...
describe("series and chapter favorites over HTTP", () => {
  test("each type has its own coordinate-addressed PUT/DELETE and membership route", async () => {
    ...
    const series = await json<FavoriteBody>(
      await send("PUT", "/library/favorites/series/demo/s9", { seriesTitle: "Nine", author: "A" }),
    );
    expect(series).toMatchObject({ type: "series", seriesId: "s9", seriesTitle: "Nine" });
    const chapter = await json<FavoriteBody>(
      await send("PUT", "/library/favorites/chapter/demo/s9/c1", { seriesTitle: "Nine", number: 1, languageCode: "en" }),
    );
    expect(chapter).toMatchObject({ type: "chapter", chapterId: "c1" });

    await send("PUT", "/library/favorites/series/demo/s9/collections", { collectionIds: [shelf.id] });
    await send("PUT", "/library/favorites/chapter/demo/s9/c1/collections", { collectionIds: [shelf.id] });

    // The mixed union comes back from one query, discriminated by `type`.
    const inShelf = await json<FavoriteBody[]>(await get(`/library/favorites?collection=${shelf.id}`));
    expect(inShelf.map((i) => i.type).sort()).toEqual(["chapter", "series"]);
    // ...and type= narrows it.
    const seriesOnly = await json<FavoriteBody[]>(await get(`/library/favorites?collection=${shelf.id}&type=series`));
  ...
  test("series/chapter PUTs merge like page PUTs do", async () => {
    ...
    const again = await json<FavoriteBody & { author?: string }>(
      await send("PUT", "/library/favorites/series/demo/s9", { seriesTitle: "Nine Renamed" }),
    );
    expect(again).toMatchObject({ seriesTitle: "Nine Renamed", author: "A" }); // omitted preserved
    await send("DELETE", "/library/favorites/series/demo/s9");
  });
});

describe("listing", () => {
  test("filters and sorts through the query string", async () => {
    await send("PUT", "/library/favorites/page/demo/s1/c2/1", { seriesTitle: "Alpha Comic", chapterName: "Ch 2" });
    await send("PUT", "/library/favorites/page/demo/s2/c9/4", { seriesTitle: "Zebra Tales", chapterName: "Ch 9" });
    await send("PUT", "/library/favorites/page/demo/s1/c1/7", { seriesTitle: "Alpha Comic", chapterName: "Ch 1" });

    const bySeries = await json<FavoriteBody[]>(await get("/library/favorites?series=demo:s1"));
    expect(bySeries.every((p) => p.seriesId === "s1")).toBe(true);
    expect(bySeries).toHaveLength(2);

    const search = await json<FavoriteBody[]>(await get("/library/favorites?q=zebra"));
    ...
    const reading = await json<FavoriteBody[]>(await get("/library/favorites?sort=chapter&dir=asc&series=demo:s1"));
    expect(reading.map((p) => p.chapterName)).toEqual(["Ch 1", "Ch 2"]);

    // An unrecognised sort falls back to the default rather than erroring.
    expect((await get("/library/favorites?sort=bogus")).status).toBe(200);

    await send("DELETE", "/library/favorites/page/demo/s1/c2/1");
    await send("DELETE", "/library/favorites/page/demo/s2/c9/4");
    await send("DELETE", "/library/favorites/page/demo/s1/c1/7");
  });
  ...
  test("favorites store no page bytes, so they add nothing to /library/usage beyond their document", async () => {
    ...
    await send("PUT", "/library/favorites/page/demo/s1/c7/0", {
      seriesTitle: "S",
    ...
    await send("DELETE", "/library/favorites/page/demo/s1/c7/0");
  });
});
...
describe("absence and namespace separation", () => {
  test("every favorite-pages route 404s when no library is mounted", async () => {
    for (const [method, path] of [
      ...
      ["GET", "/library/favorites/page/demo/s1/c1/indices"],
      ["POST", "/library/favorites/page/demo/s1/c1/reconcile"],
      ["GET", "/library/collections"],
      ["POST", "/library/collections"],
      ["PUT", "/library/favorites/series/demo/s1"],
      ["PUT", "/library/favorites/chapter/demo/s1/c1"],
      ["PUT", "/library/favorites/page/demo/s1/c1/0"],
      ["DELETE", "/library/favorites/page/demo/s1/c1/0"],
    ] as const) {
  ...
  test("page favorites never touch the bridge-account /bridges/:id/favorites namespace", async () => {
    ...
    await send("PUT", "/library/favorites/page/example/alice/c1/0", { seriesTitle: "Alice" });
    ...
    expect(await json<FavoriteBody[]>(await get("/library/favorites"))).toHaveLength(1);
    await send("DELETE", "/library/favorites/page/example/alice/c1/0");
  });
});
