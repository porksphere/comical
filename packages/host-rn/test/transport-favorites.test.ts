 ...
 * `@comical/host-server` router mounts `/library/favorites*` + `/library/collections*` and resolves
...
describe("embedded transport — page favorites", () => {
  test("mounts /library/favorites* and /library/collections* when a library is supplied", async () => {
    const t = makeTransport();

    const empty = await t("/library/favorites");
  ...
  test("leaves the favorites surface unmounted (404) when no library is supplied", async () => {
    ...
    expect((await t("/library/favorites")).status).toBe(404);
  ...
  test("favoriting round-trips in-process, and the chapter route serves the reader's indices", async () => {
    const t = makeTransport();
    expect((await put(t, "/library/favorites/page/demo/s1/c1/3", { seriesTitle: "Series One" })).status).toBe(200);
    expect((await put(t, "/library/favorites/page/demo/s1/c1/1", { seriesTitle: "Series One" })).status).toBe(200);

    const indices = await t("/library/favorites/page/demo/s1/c1/indices");
    expect(indices.status).toBe(200);
    expect(await indices.json()).toEqual([1, 3]);

    const list = (await (await t("/library/favorites")).json()) as { id: string }[];
    expect(list).toHaveLength(2);
  });

  test("reconcile reaches the router in-process and repairs a shifted favorite", async () => {
    const t = makeTransport();
    await put(t, "/library/favorites/page/demo/s1/c1/2", {
      seriesTitle: "Series One",
      pageCount: 4,
      sourceUrl: "https://cdn/p2.png",
    });

    const res = await t("/library/favorites/page/demo/s1/c1/reconcile", {
      method: "POST",
    ...
    expect(await (await t("/library/favorites/page/demo/s1/c1/indices")).json()).toEqual([1]);
  });

  test("favorites carry no stored bytes — only coordinates, snapshot and re-anchor signals", async () => {
    const t = makeTransport();
    const res = await put(t, "/library/favorites/page/demo/s1/c1/0", {
      seriesTitle: "Series One",
    ...
    expect((await t(`/library/favorites/${encodeURIComponent(String(page.id))}/thumb`)).status).toBe(404);
  });
});
