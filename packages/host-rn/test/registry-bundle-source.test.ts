/**
 * RegistryBundleSource against the REAL `@comical/registry` fetcher + verify, with a mocked global
 * `fetch` serving an index + bundle. This exercises real SHA-256 verification under Bun's WebCrypto —
 * the same `verify.ts` that runs on-device (behind a crypto.subtle polyfill).
 */
