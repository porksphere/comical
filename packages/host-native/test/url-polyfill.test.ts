...
describe("ComicalURL polyfill", () => {
  test("parses host/hostname/port/pathname/search from a typical bridge URL", () => {
    const u = new ComicalURL("https://example.test/api/favorites?page=1");
    expect(u.protocol).toBe("https:");
    expect(u.host).toBe("example.test");
  ...
  test("defaults pathname to / when absent", () => {
    expect(new ComicalURL("https://example.test").pathname).toBe("/");
  });
  ...
  test("the old href-only stub breaks CookieJar (reproduces the favorites-401)", () => {
    ...
    try {
      ...
      jar.store("https://example.test/api/auth/login", ["session=abc123; Path=/"]);
      // No host → cookie never stored → nothing to replay.
      expect(jar.header("https://example.test/api/favorites")).toBeUndefined();
    } finally {
  ...
  test("the polyfill lets CookieJar store and replay a login cookie (the fix)", () => {
    ...
    try {
      const jar = new CookieJar();
      jar.store("https://example.test/api/auth/login", ["session=abc123; Path=/; HttpOnly"]);
      expect(jar.header("https://example.test/api/favorites?page=1")).toBe("session=abc123");
    } finally {
      (globalThis as unknown as { URL: unknown }).URL = real;
    }
  });
